...
export default function HeroCarousel() {
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (delta > 50) goToPrevious(); // swipe right
    else if (delta < -50) goToNext(); // swipe left
  };
  ...
  return (
    <div
      className={`relative w-full h-[400px] sm:h-[500px] md:h-[600px] lg:h-[700px] overflow-hidden ${styles.bgLinearToR}`}
          ...
          <div className="flex gap-1">
            {progressBarLabels.map((label, index) => (
              <button key={index} onClick={() => goToSlide(index)} className="flex-1 group cursor-pointer transition-all duration-300">
                ...
                <div className="mt-3 text-left transition-all duration-300">
                  <span
                    className={`text-xs md:text-sm font-medium transition-all duration-300 ${
                      index === currentIndex ? "text-white drop-shadow-lg" : "text-white/40 group-hover:text-white/70"
                    }`}
                  >
                    {label.text}
                  </span>
        ...
        {slides.map((_, index) => (
          <button
            ...
            className={`transition-all duration-300 rounded-full ${
              index === currentIndex ? "w-2 h-2 bg-white" : "w-1.5 h-1.5 bg-white/40"
            }`}
